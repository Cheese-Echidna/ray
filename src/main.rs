...
fn coloured_spheres() -> Scene {
    let loc = Vec3::new(0.0, 2.0, 0.2);
    let camera = Camera::new_with_control(loc, Vec3::new(0.0, -1.0, 0.0), 75., 20, 300);

    println!("{:?}", camera);

    let material_ground = Diffuse::new(Vec3::new(0.8, 0.8, 0.0), 1.0);
    let material_center = Diffuse::new(Vec3::new(0.1, 0.2, 0.5), 1.0);
    let material_right = Metal::new(Vec3::new(0.8, 0.6, 0.2), 0.05);
    ...
    let mut objects = vec![
        ...
        RenderObject::new(
            Sphere::new(Vec3::new(0.0, -0.4, 0.), 0.5),
            material_center,
        ),
        RenderObject::new(Sphere::new(Vec3::new(1.0, 0.0, 0.0), 0.5), materials::clear::Clear::GLASS),
    ...
    let col: fn(Vec3, &Camera) -> Vec3Colour = |d, camera| {
        ...
        Vec3::new(0.8 - 0.3 * a, 0.8 - 0.3 * a, 1.0)
    };
