...
fn coloured_spheres() -> Scene {
    ...
    let material_center = Texture::new(
        "assets/8k_earth_day_map.jpg",
        Some("assets/8k_earth_normal_map.jpg"),
        Vec2::splat(1.0),
        0.0
    );

    let objects = vec![RenderObject::new(
        Sphere::new(Vec3::new(0., 0., 0.), 0.8),
        material_center,
    )];
